            ...
            <table class="table table-striped table-bordered table-hover" id="table1">
                ...
                <tr>
                    ...
                    <td style="color: #428bca">
                        <?php switch($vo['status']): case "0": ?>待支付<?php break; case "1": ?><i class="fa fa-taxi" onclick="fahuo('<?php echo $vo['id']; ?>')" >&nbsp;发货</i>
                        <?php break; case "2": ?>待收货<?php break; case "3": ?>待评价<?php break; case "4": ?>已完成<?php break; case "5": ?><i class="fa fa-thumbs-up" style="cursor: pointer" onclick="ttk('<?php echo $vo['id']; ?>')">同意退款</i>&nbsp;&nbsp;&nbsp;&nbsp;<i
                            class="fa fa-thumbs-down" onclick="ju_tk('<?php echo $vo['id']; ?>')" style="cursor: pointer">拒绝退款</i><?php break; case "7": ?>已拒绝退款<?php break; case "8": ?><i class="fa fa-thumbs-up" style="cursor: pointer" onclick="ttk('<?php echo $vo['id']; ?>')">同意退货</i>&nbsp;&nbsp;&nbsp;&nbsp;
                        <i class="fa fa-thumbs-down" onclick="ju_th('<?php echo $vo['id']; ?>')" style="cursor: pointer">拒绝退货</i><?php break; case "9": ?>已拒绝退货<?php break; endswitch; ?>
                    </td>
                    <td>
                        <?php echo show_operate($vo['id'],'update','查 看'); ?>
                        &nbsp;&nbsp;&nbsp;&nbsp;
...
<script>
    function fahuo(id) {
        ...
        $(".tj").click(function () {
            ...
            $.post(
                "<?php echo url('RefundMoney/fahuo'); ?>",{id:id,courier_name:name,courier_number:member},function (data) {
                    layer.msg(data.msg);
                    setTimeout(function () {
                        location.reload();
                    },1500)
    ...
    function ttk(id) {
        $.post(
            "<?php echo url('RefundMoney/achieve'); ?>", {id: id}, function (data) {
                layer.msg(data.msg);
                setTimeout(function () {
                    location.reload();
                }, 1000);
    ...
    function ju_tk(id) {
        $.post(
            "<?php echo url('RefundMoney/reject_tk'); ?>", {id: id}, function (data) {
                layer.msg(data.msg);
                setTimeout(function () {
                    location.reload();
                }, 1000);
    ...
    function ju_th(id) {
        $.post(
            "<?php echo url('RefundMoney/reject_th'); ?>", {id: id}, function (data) {
                layer.msg(data.msg);
                setTimeout(function () {
                    location.reload();
                }, 1000);
              ...
              <div class="form-group">
               ...
               <div class="form-group">
                    ...
                    <div class="col-sm-6 avatar-size">
                        ...
                        <?php $__FOR_START_2001__=1;$__FOR_END_2001__=9;for($i=$__FOR_START_2001__;$i < $__FOR_END_2001__;$i+=1){ ?>
