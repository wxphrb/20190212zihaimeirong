...
use think\Request;
use think\Url;
use app\console\model\BbcType as ThisModel;

class BbcType extends Base
{
    /**
     * [index description]列表
     * @return [type] [description]
     */
    public function index(Request $request, ThisModel $bbcType)
    {
        $data = $bbcType
            ->order("id", "desc")
            ->paginate(10);
    ...
    public function create()
    {
        if (Request::instance()->isPOST()) {
            ...
            if (true === $result) {
                $this->success('新建成功', 'console/BbcType/index');
            } else {
                $this->error($result);
            }
        }
    ...
    public function update($id)
    {
        if (Request::instance()->isPOST()) {
            ...
            if (true === $result) {
                $this->success('更新成功', 'console/BbcType/index');
            } else {
                $this->error($result);
            }
        }
    ...
    public function delete()
    {
        if (Request::instance()->isPOST()) {
            ...
            if ($delmodel) {
                $this->success('删除成功', 'console/BbcType/index');
            } else {
