...
use think\Request;
use think\Url;
use app\console\model\PostsComments as ThisModel;
use app\console\model\User;

class PostsComments extends Base
{
    /**
     * [index description]列表
     * @return [type] [description]
     */
    public function index(Request $request, ThisModel $comments)
    {
        $id = $request->param("id");
        $name = array();
        $where = ['posts_id' => $id, 'parent_id' => 0];
        if ($request->isPost()) {
