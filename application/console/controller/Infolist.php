<?php

namespace app\console\controller;

use think\Db;

use think\Request;

use think\Url;

use app\console\model\Infolist as ThisModel;
use app\console\model\NewsInfo;


class Infolist extends Base

{

    /**

	 * [index description]列表

	 * @return [type] [description]

	 */

	public function index(ThisModel $infolist)

	{

//		$data = ThisModel::order('id', 'desc')->select();



		$data = $infolist

            ->relation(['approve'])

            ->order('id', 'DESC')

            ->paginate(20);



        return $this->fetch('index', [

            'list'       => $data

        ]);

	}

	/**

     * [create description]添加方法

     * @return [type] [description]

     */

	public function create()

	{

		if (Request::instance()->isPOST())

		{



			$data = Request::instance()->post();

            $data['create_time'] = time();

			$result = ThisModel::saveVerify($data);

			if (true === $result) {

                $this->success('新建成功', 'console/Infolist/index');

            } else {

                $this->error($result);

            }

		}

		$param = $this->appendarg();



		return $this->fetch('create', $param);

	}



    /**

     * [update description]更新方法

     * @param  [type] $id [description]主键id

     * @return [type]     [description]

     */

	public function update($id,NewsInfo $newsInfo)

	{

		if (Request::instance()->isPOST())

		{

			$data = Request::instance()->post();

			$result = ThisModel::saveVerify($data,$id);
            if($result){
				$res = Db::name('certification')->where('id',$data['id'])->find();
				if($data['status']=='2'){
				    Db::name('user')->where('id',$res['user_id'])->setField('level_id',$res['certification_type']);
					$request['title'] = "您的身份认证已经通过审核！";
				}elseif($data['status']=='3'){
					$request['title'] = "您的身份认证未通过审核！";
				}
				$request['type'] = 6;
				$newsInfo->isUpdate(false)->allowField(true)->save([
					'user_id' => $res['user_id'],'type' =>6,'title' => $request['title'],'status'=>2,'create_time'=>time()
				]);
				push_user($request['title'], $res['user_id'],0, 6, $newsInfo->id);
			}
			if (true === $result) {

                $this->success('更新成功', 'console/Infolist/index');

            } else {

                $this->error($result);

            }

		}



		$data = ThisModel::get($id);

		$param = array_merge(['vo'=>$data], $this->appendarg());

		return $this->fetch('create', $param);

	}

    /**

     * 添加修改时候需要传递参数的话，用此方法，只写一遍

     */

	public function appendarg(){

		$infoclass = db::name('certification_type')->field('id,title')->select();

		return [

		   'infoclass' =>$infoclass,

		];

	}

    /**

     * [delete description]删除方法 多选和单选删除

     * @return [type] [description]

     */

	public function delete(){

		if (Request::instance()->isPOST())

		{

			$id = Request::instance()->post('id/a'); // (/a)方法 将收到的id转为数组

			$delmodel = ThisModel::destroy($id);

			if($delmodel){

			    $this->success('删除成功', 'console/Infolist/index');

			}

			else{

				$this->error($delmodel->getError());

			}

	    }

	    else{

	    	$this->error('请求方式出错!');

	    }

	}

    /**

     * [renewfield description]列表更新字段

     * @return [type] [description]

     */

	public function renewfield(){

		if (Request::instance()->isPOST())

		{

            $data = Request::instance()->post();

			$validate = validate('Infolist');



			$post = Request::instance()->except(['id'],'post');

			$post = array_keys($post);



            $validate->scene('edit', $post);

			if(!$validate->scene('edit')->check($data)){

			    $this->error($validate->getError());

			}

	        $this_model = new ThisModel();

	        if($this_model->update($data))

            {

			    $this->success('更新成功', 'console/Infolist/index');

			}

			else{

				$this->error($this_model->getError());

			}

		}

	    else{

	    	$this->error('请求方式出错!');

	    }

	}

}

