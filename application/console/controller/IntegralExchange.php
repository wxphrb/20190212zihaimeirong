...
use think\Request;
use think\Url;
use app\console\model\IntegralExchange as ThisModel;
...
class IntegralExchange extends Base
{
     ...
     */
    public function index(ThisModel $exchange)
    {
        $name = array();
        $where = array();
        ...
        $data = $exchange
            ...
            ->paginate(20, false, ['query' => array('name' => $name)]);
    ...
    public function create()
    {
        if (Request::instance()->isPOST()) {
            ...
            if (true === $result) {
                $this->success('新建成功', 'console/Integral_exchange/index');
            } else {
                $this->error($result);
            }
        }
    ...
    public function update($id)
    {
        if (Request::instance()->isPOST()) {
            ...
            if (true === $result) {
                $this->success('更新成功', 'console/Integral_exchange/index');
            } else {
                $this->error($result);
            }
        }
    ...
    public function delete()
    {
        if (Request::instance()->isPOST()) {
            ...
            if ($delmodel) {
                $this->success('删除成功', 'console/Integral_exchange/index');
            } else {
    ...
    public function renewfield()
    {
        if (Request::instance()->isPOST()) {
            $data = Request::instance()->post();
            $validate = validate('Integral_exchange');
    ...
    /*发货*/
    public function logistics(Request $request, ThisModel $exchange)
    {
        $data = $request->param();
        $data['status'] = 1;
        $result = ThisModel::saveVerify($data, $data['id']);
        if (true === $result) {
            $this->success('更新成功', 'console/Integral_exchange/index');
        } else {
            $this->success('更新失败');
