...
use think\Request;
use think\Url;
use app\console\model\JobType as ThisModel;
...
class JobType extends Base
{
	 ...
	 */
	public function index(ThisModel $jobType)
	{
        $name = array();
        $where = array();
        ...
        $data = $jobType->where($where)->order('id desc')->paginate(30, false, ['query' => array('name' => $name)]);
	...
	public function create()
	{
		if (Request::instance()->isPOST())
		{
			...
			if (true === $result) {
                $this->success('新建成功', 'console/JobType/index');
            } else {
                $this->error($result);
            }
		}
	...
	public function update($id)
	{
		if (Request::instance()->isPOST())
		{
			...
			if (true === $result) {
                $this->success('更新成功', 'console/JobType/index');
            } else {
                $this->error($result);
            }
		}
	...
	public function delete(){
		if (Request::instance()->isPOST())
		{
			...
			if($delmodel){
			    $this->success('删除成功', 'console/JobType/index');
			}
			else{
