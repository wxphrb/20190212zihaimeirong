...
use app\shop\model\Orders as OrderModel;

use app\shop\model\NewsInfo;
...

    {
        ...
        if (Request::instance()->isPOST()) {
            ...
            if (true === $result) {
                ...
                $this->success('更新成功', 'shop/Deliver/index');
...

    {
        ...
        if (Request::instance()->isPOST()) {
            ...
            if ($delmodel) {
                ...
                $this->success('删除成功', 'shop/Deliver/index');
...

    {
        ...
        if (Request::instance()->isPOST()) {
            ...
            if ($this_model->update($data)) {
                ...
                $this->success('更新成功', 'shop/Orders/index');
...
//        $data['courier_number'] = $request->param("courier_member");
        ...
        if ($result) {
            ...
            $map['type'] = 7;
            $newsInfo->isUpdate(false)->allowField(true)->save([
                'other_id'=>$data['id'], 'user_id' => $res['user_id'],'type' =>7,'title' => $map['title'],'status'=>2,'create_time'=>time()
            ]);
            push_user($map['title'], $res['user_id'],$res['id'], 7, $newsInfo->id);
            return ["msg" => "更新成功"];
